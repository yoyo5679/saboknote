try {
                ...
                try {
                    ...
                    if (container) {
                        if (error || !data || data.length === 0) {
                            container.innerHTML = '<div style="text-align:center; padding:20px; color:#64748b; font-size:0.9rem;">아직 도착한 비밀 편지가 없어요.</div>';
                        } else {
                            ...
                            data.forEach((post, i) => {
                                ...
                                <div style="margin-bottom:12px; padding-bottom:12px; border-bottom:1px dashed #ddd6fe; cursor:pointer;" onclick="showNewsletterDetail('${post.id}')">
                                    <strong style="color:#0f172a; font-size:0.95rem; display:block; margin-bottom:4px">[Vol ${data.length - i}.] ${escapeHtml(post.title)}</strong>
    ...
    window.showNewsletterDetail = function (id) {
        ...
        const contentHtml = `
            <div style="background:#f8fafc; padding:20px; border-radius:16px; border:1px solid #e2e8f0; margin-bottom:20px;">
                <div style="margin-bottom:16px; display:flex; justify-content:space-between; align-items:center; border-bottom:2px solid var(--primary); padding-bottom:12px;">
                    <strong style="color:#1e293b; font-size:1.1rem;">${escapeHtml(post.title)}</strong>
                </div>
                <div style="font-size:0.95rem; line-height:1.7; color:#334155; white-space:pre-wrap;">${escapeHtml(post.content)}</div>
            </div>
        ...
        openModal(post.title, contentHtml);
    }
    /* --- 감정 파쇄기 로직 --- */
...
감사합니다.`;
            ...
            window.activeGeneratedNewsletter = { title, body };
            ...
            <div style="animation: fadeIn 0.3s ease">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; border-bottom:2px solid var(--primary); padding-bottom:10px">
                    ...
                    <div style="display:flex; gap:8px;">
                        ...
                        <button id="nl-publish-btn" class="btn-primary" style="background:#7c3aed; padding:6px 12px; font-size:0.8rem; border-radius:8px;" onclick="publishNewsletter()">DB에 발행하기 🚀</button>
                    </div>
                </div>
...
${body}</div>
    ...
    window.publishNewsletter = async function () {
        if (!window.activeGeneratedNewsletter || !supabase || !myUserId) {
            alert('발행할 데이터가 없거나 로그인/디비 연동이 안 되어 있습니다.');
            return;
        }

        const { title, body } = window.activeGeneratedNewsletter;
        const btn = document.getElementById('nl-publish-btn');
        if (btn) { btn.disabled = true; btn.innerText = '발행 중...'; }
        ...
        try {
            ...
            alert('비밀편지가 성공적으로 발행되었습니다! 🎉');
            closeModal();

        } catch (e) {
            console.error('Publish error:', e);
            alert('발행 중 오류가 발생했습니다.');
            if (btn) { btn.disabled = false; btn.innerText = 'DB에 발행하기 🚀'; }
        }
    };
    ...
    window.initMypage = async function () {
        ...
        try {
            const { data: qaData } = await supabase
                .from('qa_posts')
                .select('parent_id, likes')
                .eq('user_id', myUserId);

            if (qaData) {
                qaData.forEach(p => {
                    if (!p.parent_id) qCount++;
                    else aCount++;
                    if (p.likes) thanksCount += p.likes;
                });
            }
