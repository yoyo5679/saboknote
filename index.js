...
{{INPUT}}`
            ...
            <div style="margin-top:20px; padding:16px; background:#fff1f2; border-radius:12px; border:1px solid #ffe4e6; text-align:left;">
                ...
                <div id="admin-content-youth" class="tab-content" style="display:none; animation: fadeIn 0.3s ease;">
                    <div class="step-card beautiful-card" style="margin-bottom:24px; padding:20px;">
                        ...
                        <div style="background:#f8fafc; padding:16px; border-radius:12px; border:1px solid #e2e8f0; margin-bottom:20px;">
                            <div style="display:flex; flex-direction:column; gap:16px;">
                                <div>
                                    ...
                                    <input type="date" id="youth-end-date" class="calc-input" onchange="calcYouthIndependence()" style="font-size:1.1rem; padding:12px;">
                                </div>
                                <div>
                                    <label style="display:block; font-size:0.85rem; color:#475569; font-weight:700; margin-bottom:6px;">기준일 (미입력 시 '오늘')</label>
                                    <input type="date" id="youth-base-date" class="calc-input" onchange="calcYouthIndependence()" style="font-size:1.1rem; padding:12px;">
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Military Option -->
                        <div style="margin-bottom:20px;">
                            <label style="display:flex; align-items:center; gap:8px; cursor:pointer;" onchange="toggleMilitaryInput()">
                                <input type="checkbox" id="youth-military-check" style="width:18px; height:18px; accent-color:#2563eb;">
                            ...
                            <div id="youth-military-input" style="display:none; margin-top:12px; background:#eff6ff; padding:16px; border-radius:12px; border:1px dashed #bfdbfe;">
                                ...
                                <div style="display:flex; gap:12px;">
                                    <div style="flex:1;">
                                        ...
                                        <input type="date" id="youth-mil-start" class="calc-input" onchange="calcYouthIndependence()" style="font-size:1.1rem; padding:12px;">
                                    </div>
                                    <div style="flex:1;">
                                        <label style="display:block; font-size:0.8rem; color:#1e40af; font-weight:700; margin-bottom:4px;">전역(예정)일</label>
                                        <input type="date" id="youth-mil-end" class="calc-input" onchange="calcYouthIndependence()" style="font-size:1.1rem; padding:12px;">
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    window.switchAdminTab = function (tabName) {
        ...
        setInactive(btnVat); setInactive(btnTax); setInactive(btnLtc); setInactive(btnPayroll); setInactive(btnBudget); setInactive(btnYouth);
...
• <strong>이메일 주소</strong>: 비밀 편지(뉴스레터) 구독 신청 시 이용자가 직접 입력하는 경우에만 수집. <span style="color:#ef4444; font-weight:700;">동의 없이 수집하지 않습니다.</span></p>
    ...
    function pgRenderResultPage() {
        ...
        const html = `
            ...
            <button id="pg-btn-img-copy" onclick="pgHandleCopyImage()" style="${btnStyle(t.bg, `0 6px 20px ${t.color}50`)} display:flex; align-items:center; justify-content:center; gap:8px;">
              🖼️ 결과 이미지 복사하기
            </button>
            <div style="text-align:center; font-size:11px; color:#bbb; margin-bottom:8px;">
              복사 후 카카오톡·인스타 채팅창에 붙여넣기(Ctrl+V) 하세요 🙌
            </div>
    ...
    window.pgHandleCopyImage = async function () {
        if (!pgState.resultType) return;
        const btn = document.getElementById('pg-btn-img-copy');
        try {
            await pgCopyImageToClipboard(pgTypes[pgState.resultType]);
            if (btn) {
                const oldBg = btn.style.background;
                btn.style.background = "#4CAF50";
                btn.innerHTML = "✅ 이미지 복사됨! 카톡·인스타에 붙여넣기 하세요";
                setTimeout(() => { btn.style.background = oldBg; btn.innerHTML = "🖼️ 결과 이미지 복사하기"; }, 3000);
            }
        } catch (e) {
            alert("이미지 복사에 실패했어요.\n(모바일 기기나 특정 브라우저 환경에서는 클립보드 이미지 복사가 제한될 수 있습니다.)");
            console.error(e);
        }
    };

    async function pgCopyImageToClipboard(t) {
        const W = 800, H = 800;
