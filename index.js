...
{{INPUT}}`
                        ...
                        <div style="background:rgba(255,255,255,0.7); backdrop-filter:blur(10px); border-radius:20px; padding:20px; margin-bottom:24px; border:1px solid rgba(226,232,240,0.8); box-shadow:0 10px 15px -3px rgba(0,0,0,0.05);">
                            ...
                            <div style="display:grid; grid-template-columns: repeat(2, 1fr); gap:12px; margin-bottom:16px;">
                                <div style="background:#f8fafc; padding:12px; border-radius:12px; border:1px solid #e2e8f0;">
                                    <label style="font-size:0.7rem; color:#64748b; font-weight:700; display:block; margin-bottom:4px;">기본급 (세전)</label>
                                ...
                                <div style="background:#f8fafc; padding:12px; border-radius:12px; border:1px solid #e2e8f0;">
                                    <label style="font-size:0.7rem; color:#64748b; font-weight:700; display:block; margin-bottom:4px;">가족수당</label>
                                ...
                                <div style="background:#f8fafc; padding:12px; border-radius:12px; border:1px solid #e2e8f0;">
                                    <label style="font-size:0.7rem; color:#64748b; font-weight:700; display:block; margin-bottom:4px;">정액급식비</label>
                                ...
                                <div style="background:#f0f9ff; padding:12px; border-radius:12px; border:1px solid #bae6fd;">
                                    <label style="font-size:0.7rem; color:#0369a1; font-weight:700; display:block; margin-bottom:4px;">통상시급 (원)</label>
                            ...
                            <div style="display:flex; gap:8px;">
                                <div style="flex:1; background:#f1f5f9; padding:10px; border-radius:10px; text-align:center;">
                                    <label style="font-size:0.65rem; color:#64748b; font-weight:700; display:block;">연장시간</label>
                                ...
                                <div style="flex:1; background:#f1f5f9; padding:10px; border-radius:10px; text-align:center;">
                                    <label style="font-size:0.65rem; color:#64748b; font-weight:700; display:block;">야간시간</label>
                                ...
                                <div style="flex:1; background:#f1f5f9; padding:10px; border-radius:10px; text-align:center;">
                                    <label style="font-size:0.65rem; color:#64748b; font-weight:700; display:block;">휴일시간</label>
                            ...
                            <div style="margin-top:16px;">
                                ...
                                <div style="display:flex; gap:8px;">
                                    <button id="ltc-gasan-0" onclick="setLtcGasan(0)" style="flex:1; padding:10px 4px; border-radius:8px; border:1px solid #e2e8f0; background:#5cb85c; color:white; font-size:0.75rem; font-weight:700; cursor:pointer; transition:all 0.2s;">기본(0%)</button>
                                    <button id="ltc-gasan-30" onclick="setLtcGasan(30)" style="flex:1.5; padding:10px 4px; border-radius:8px; border:1px solid #e2e8f0; background:#f8fafc; color:#475569; font-size:0.75rem; font-weight:700; cursor:pointer; transition:all 0.2s;">심야, 휴일(30%)</button>
                                    <button id="ltc-gasan-50" onclick="setLtcGasan(50)" style="flex:1.5; padding:10px 4px; border-radius:8px; border:1px solid #e2e8f0; background:#f8fafc; color:#475569; font-size:0.75rem; font-weight:700; cursor:pointer; transition:all 0.2s;">유급휴일(50%)</button>
                                </div>
                                <input type="hidden" id="ltc-gasan-value" value="0">
                            </div>
                        </div>
    ...
    window.calcPayrollTax = function () {
        ...
        if (baseSalary === 2906000 && familyAllowance === 140000 && mealAllowance === 130000 && otHours === 1 && hourlyRate === 15899) {
            ...
            // ➕ 가산 반영 (LTC Addition)
            const gasanRate = parseFloat(document.getElementById('ltc-gasan-value').value) / 100;
            // 장기요양보험료 (건보료의 12.95% 수준) * (1 + 가산율)
            eeCare = Math.floor(eeHealth * 0.12945 * (1 + gasanRate));
    ...
    window.switchAdminTab = function (tabIndex) {
        document.querySelectorAll('.admin-tab').forEach(tab => {
            tab.style.borderBottom = 'none';
            tab.style.color = '#64748b';
            tab.style.fontWeight = '500';
        });
        const activeTab = document.querySelectorAll('.admin-tab')[tabIndex - 1];
        ...
        document.getElementById(`admin-section-${tabIndex}`).style.display = 'block';
    };

    window.setLtcGasan = function (value) {
        document.getElementById('ltc-gasan-value').value = value;

        // UI Update
        [0, 30, 50].forEach(v => {
            const btn = document.getElementById(`ltc-gasan-${v}`);
            if (v === value) {
        ...
        if (typeof calcPayrollTax === 'function') calcPayrollTax();
    };
